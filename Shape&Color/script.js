...
function calculateImageArea() {
    ...
    // Apply threshold (Adaptive thresholding might work better)
    cv.threshold(blurred, binary, 128, 255, cv.THRESH_BINARY_INV);

    // Morphology close to fill gaps inside objects
    ...
    let totalArea = 0;
    let minContourArea = 50; // Ignore small noise
    for (let i = 0; i < contours.size(); i++) {
    ...
    // Clone and display the image with text overlay
    let dst = src.clone();
    cv.putText(dst, `Total Area: ${Math.round(totalArea)} px`, 
        new cv.Point(10, 30), cv.FONT_HERSHEY_SIMPLEX, 
        0.7, new cv.Scalar(0, 255, 0, 255), 2);

    // Show updated image
    cv.imshow(canvas, dst);
    document.getElementById('imageArea').textContent = Math.round(totalArea);

    // Cleanup
    dst.delete(); src.delete(); gray.delete(); blurred.delete(); binary.delete();
...
function detectCentroids() {
    ...
    function segmentation() {
        // Read image from originalCanvas
        ...
        // Apply binary thresholding for basic segmentation
        cv.threshold(gray, binary, 120, 255, cv.THRESH_BINARY);
    
        // Show result on editedCanvas
        cv.imshow('editedCanvas', binary);
